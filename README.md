...
2. **セッション管理（複数回点呼対応）**: 管理画面の「セッション締め」ボタンで現在の点呼データをアーカイブし、重複チェックをリセット。午前/午後など同じ日に複数回の点呼が可能。締め済みデータは「セッション履歴」タブから閲覧・ダウンロード可能。
3. **Power Automate 連携による無料メール送信**: 点呼記録時に `s{last5}@tokyo.kosen-ac.jp` 宛てに完了通知メールを送信（管理画面から自動送信/手動送信の切り替え、一括・個別送信が可能）。
4. **Web 管理ビュー（ダッシュボード）**:
   - ブラウザから安全にログイン（パスワード認証 & レートリミット保護）。
   - リアルタイム点呼状況・完了率・拠点別集計の可視化。
   - **未点呼者リスト**: 学生マスタと現在のセッションの点呼データを突合し、未完了学生を瞬時に一覧表示。
