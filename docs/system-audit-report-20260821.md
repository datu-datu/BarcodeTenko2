...
| BUG-02 ✅対応済 | 設計不具合 | サーバー重複排除ルール（同日+同番号）が「午前/午後」複数回点呼と矛盾 | **High** | 中 |
...
2. **運用開始前までに**
   - ✅ BUG-02: 同日複数回点呼を許す重複判定仕様の決定と実装 → **対応済み (サーバー側セッション締め + アーカイブ方式)**
...
| BUG-02 (2026-08-21 追補) | **サーバー側セッション締め方式**を導入。①`ArchivedScans` テーブル新設（既存 DB 向けに起動時 `CREATE TABLE IF NOT EXISTS` パッチ）、②`POST /api/v1/dashboard/sessions/close`（トランザクションで Scans→ArchivedScans へ全行退避、任意ラベル付き）、③`GET sessions` 一覧、④エクスポートへ `session` パラメータ追加＋日付指定時はアーカイブ合算。**PostScans の重複判定は無変更**（Scans=アクティブセッションのみになるため自動的に「セッション内重複」へ変わる）。未点呼者はユーザー決定により**セッション単位**（締め後リセット）。UI は「セッション締め」ボタン + 確認ダイアログ + 「セッション履歴」タブを追加 | 結合テスト `DashboardController_SessionClose_ArchivesAndAllowsRescan` を追加（テスト用学生マスタ暗号化ヘルパー新設）。実機 E2E: 午前スキャン→締め(movedCount=1)→**同一学生の午後再スキャンが acceptedCount=1・通知キュー投入**→summary はアクティブのみ(1件)→sessions 一覧に「午前」表示、を確認。テスト **15/15 合格** |

**BUG-02 対応の設計メモ**:
- データフロー: `Scans`(アクティブ) →[セッション締め]→ `ArchivedScans`(SessionId/Label/ClosedAt 付与)。
- クライアント (Tenko.Native) は**一切変更なし**（セッション ID 送信なし）。クライアント側の「点呼完了」とサーバーの「セッション締め」は独立に運用可能。
- summary / scans / unverified はアクティブセッション基準。過日データは「セッション履歴」タブまたは `export?session=` / `export?date=`（アーカイブ合算）で参照。
- 副次効果: 従来は締め操作なしで同日再スキャンが全員破棄されメールも飛ばなかったが、午後点呼でも完了メールが正しく送信されるようになった。
- StudentMasterService の探索ロジックを改善（enc+passphrase の同一ディレクトリペアを優先。従来は別ディレクトリを掛け合わせて復号失敗し得た）。
